...
#define CLI_REQ_TIME		\
			RKH_TIME_SEC( (bsp_rand() % 5) + 2 )
#define CLI_USING_TIME		\
			RKH_TIME_SEC( (bsp_rand() % 5) + 1 )
...
{
	rkh_tim_init( &CAST(CLI_T, sma)->cli_utmr, &e_touse, NULL );
	rkh_tim_init( &CAST(CLI_T, sma)->cli_rtmr, &e_toreq, NULL );
}
...
{
	rkh_tim_stop( &CAST(CLI_T, sma)->cli_rtmr );
...
cli_delay_req( const struct rkhsma_t *sma )
{
	...
	time = CLI_REQ_TIME;
	rkh_tim_oneshot( &CAST(CLI_T, sma)->cli_rtmr, sma, time );
	bsp_wait_req( RKH_GET_PRIO(sma), time/BSP_TICKS_PER_SEC );
}
...
cli_req( const struct rkhsma_t *sma, RKHEVT_T *pe )
{
	...
	e_req->clino = sma->romrkh->prio;
	RKH_SMA_POST_FIFO( svr, (RKHEVT_T*)e_req, sma );
	bsp_req( e_req->clino );
}
...
cli_start( const struct rkhsma_t *sma, RKHEVT_T *pe )
{
	...
	(void)pe;
	time = CLI_USING_TIME;
	rkh_tim_oneshot( &CAST(CLI_T, sma)->cli_utmr, sma, time );
	bsp_using( RKH_GET_PRIO(sma), time/BSP_TICKS_PER_SEC );
}
