 ...
 *	RKC to deal with multiple event-driven state machines.
 *
 *	The RKC executes one state machine application or active object at a time 
 *	in the infinite loop (similar to the "superloop"). The RKC is engaged 
 ...
 *	event. The RKC scheduler is cooperative, which means that all active 
 ...
 *	of the RKC, every ISR returns to exactly the preemption point. If the ISR 
 ...
 *	task-level response. With the nonpreemptive RKC, the task-level response 
